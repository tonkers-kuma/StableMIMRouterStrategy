...
import "./MIMMinterRouterStrategy.sol";

contract MIMMinterRouterFactory {
    ...
    bool _underlying_is_lp) public {
        MIMMinterRouterStrategy _original = new MIMMinterRouterStrategy(_vault,
        _yVault,
        _strategyName,
        _abracadabra,
        _maxCollatRate,
    ...
    function name() external view returns (string memory) {
        return
            string(
                abi.encodePacked(
                    "Factory",
                    MIMMinterRouterStrategy(payable(original)).name(),
                    "@",
                    MIMMinterRouterStrategy(payable(original)).apiVersion()
                )
            );
    }
